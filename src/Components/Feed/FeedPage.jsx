...
const FeedPage = () => {
    ...
    const {
        activityState: { feeds, loading },
        dispatchActivity,
    } = useActivityContext();
    const sortedNonArchivedFeed = feeds
        .filter((feed) => feed.direction)
        .slice()
        .sort((a, b) => {
        ...
        <div>
            ...
            {(() => {
                if (loading) return <Loader amount={6} key={'feedloader'} />;
                    ...
                    return (
                        <Fragment>
                            {sortedNonArchivedFeed.map((feed) => (
                                <FeedItem
                                    key={`feeditem-${feed.id}`}
                                    feed={feed}
                                />
                            ))}
                        </Fragment>
